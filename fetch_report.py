...
creative_meta_by_ad_id = {a['id']: (a.get('creative') or {}) for a in ads_meta_raw}

# Видео нельзя получить обычным insights-запросом creative — у AdCreative
# есть только video_id, а само проигрываемое видео (source) отдаёт отдельный
# узел Video, и то не всегда (зависит от прав токена и типа видео). Если
# source недоступен — фронтенд обязан показать превью + бейдж VIDEO, а не
# генерировать поддельный URL.
video_ids = sorted({
    meta.get('video_id') for meta in creative_meta_by_ad_id.values() if meta.get('video_id')
})


def fetch_video_sources(ids, failed_sections):
    sources = {}
    chunk_size = 50
    for i in range(0, len(ids), chunk_size):
        ...
        try:
            payload = _request_with_retry(
                ...
                {'ids': ','.join(chunk), 'fields': 'source', 'access_token': ACCESS_TOKEN},
                'videos', 'videos',
            )
        except MetaAPIError as e:
        ...
        if isinstance(payload, dict):
            for video_id, data in payload.items():
                if isinstance(data, dict) and data.get('source'):
                    sources[video_id] = data['source']
    return sources


video_source_by_id = fetch_video_sources(video_ids, failed_sections) if video_ids else {}
...
for c in creatives:
    ...
    c["video_id"] = video_id
    c["video_source"] = video_source_by_id.get(video_id) if video_id else None
