...
INVISIBLE_CHARS_RE = re.compile('[' + ''.join(chr(cp) for cp in _INVISIBLE_CODEPOINTS) + ']')
...
def clean_invisible(text):
    ...
    содержит "квал" с невидимым символом на конце)."""
...
# Реальные выгрузки CRM встречались как с секундами, так и без них, и как в
# формате dd.mm.YYYY, так и в ISO-подобном YYYY-mm-dd — пробуем все варианты
# по очереди, а не падаем на первом же несовпадении.
CRM_DATE_FORMATS = (
    '%d.%m.%Y %H:%M:%S',
    '%d.%m.%Y %H:%M',
    '%Y-%m-%d %H:%M:%S',
...
def fetch_csv(url):
    ...
    resp.encoding = 'utf-8'
    return list(csv.reader(io.StringIO(resp.text))), resp.status_code


def parse_crm_rows(rows):
    """Разбирает строки CRM CSV (без заголовка) в список лидов плюс
    диагностику. Вынесено из main(), чтобы можно было протестировать без
    сети — main() лишь оборачивает это сетевым fetch и записью файла."""
    leads = []
    ...
    for row in rows:
        ...
        leads.append({
            ...
            # Сохраняем сырую строку только когда парсинг реально не удался —
            # это нужно, чтобы не молчать о потере даты (см. issue #19) и
            # иметь возможность диагностировать формат без повторного похода
            # в CRM.
            "created_at_raw": raw_date if (raw_date and created_at is None) else None,
    ...
    return {
        "leads": leads,
        "suspicious_qual_values": suspicious_qual_values,
...
def build_data_quality(leads, source_url, http_status, csv_row_count, duplicates,
                        rows_with_unparseable_date, qualified_rows_with_unparseable_date):
    """Считает диагностику по уже распарсенным лидам (created_at ещё datetime
    или None, вызывается до сериализации в строки)."""
    total = len(leads)
    ...
    qualified_with_date = sum(1 for l in leads if l["qualified"] is True and l["created_at"] is not None)
    qualified_without_date = qualified_count - qualified_with_date

    dated = [l["created_at"] for l in leads if l["created_at"] is not None]
    qualified_dated = [l["created_at"] for l in leads if l["qualified"] is True and l["created_at"] is not None]
    latest_created_at = max(dated).strftime('%Y-%m-%d %H:%M:%S') if dated else None
    latest_qualified_created_at = max(qualified_dated).strftime('%Y-%m-%d %H:%M:%S') if qualified_dated else None

    return {
        "source_url": source_url,
        "http_status": http_status,
        "csv_row_count": csv_row_count,
        "total_crm_leads": total,
        ...
        "latest_qualified_created_at": latest_qualified_created_at,
    }


def main():
    rows, crm_http_status = fetch_csv(CSV_URL)
    ...
    parsed = parse_crm_rows(rows[1:])
    leads = parsed["leads"]
    suspicious_qual_values = parsed["suspicious_qual_values"]
    duplicates = parsed["duplicates"]
    rows_with_unparseable_date = parsed["rows_with_unparseable_date"]
    qualified_rows_with_unparseable_date = parsed["qualified_rows_with_unparseable_date"]

    lead_form_entries = []
    for sheet_id in LEAD_FORM_SHEETS:
        url = f'https://docs.google.com/spreadsheets/d/{sheet_id}/export?format=csv&gid=0'
        try:
            sheet_rows, _ = fetch_csv(url)
    ...
    data_quality = build_data_quality(
        leads, CSV_URL, crm_http_status, len(rows) - 1, duplicates,
        rows_with_unparseable_date, qualified_rows_with_unparseable_date,
    )

    if data_quality["qualified_without_date"]:
        print(f"ВНИМАНИЕ: {data_quality['qualified_without_date']} qualified-лидов не попали ни в одну дату (created_at не распарсился) — они не молча исчезают, а учтены в data_quality.qualified_without_date.")
    ...
    print(f"Квал: {data_quality['qualified']} квал, {data_quality['not_qualified']} не квал, {data_quality['pending']} без статуса. Дубликатов deal_id пропущено: {duplicates}. Не распарсилась дата в {rows_with_unparseable_date} строках (из них {qualified_rows_with_unparseable_date} квал).")
